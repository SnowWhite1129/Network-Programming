...
for i in $( seq ${TEST_CASE_START} 15 ); do
  make
  echo "[1;34m===== Test case ${i} =====[m"
  cd work_dir
  rm -f ../output/${i}.txt
  env -i stdbuf -o 0 -e 0 ${SHELL_PATH} < ../demoData/testcase/${i}.txt > ../output/${i}.txt 2>&1
  cd ..
  diff -w output/${i}.txt demoData/answer/${i}.txt > /dev/null
  if [ $? -eq 0 ]; then
